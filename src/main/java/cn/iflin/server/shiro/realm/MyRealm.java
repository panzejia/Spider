package cn.iflin.server.shiro.realm;
...
import cn.iflin.spider.model.UserModel;
...
//        System.out.println(ReflectionToStringBuilder.toString(token, ToStringStyle.MULTI_LINE_STYLE));
        ...
        UserModel um = getUserInfo(token.getUsername());
        if(um!=null){
        	AuthenticationInfo authcInfo = new SimpleAuthenticationInfo(token.getUsername(),um.getPassword(), this.getName());
            this.setAuthenticationSession(um.getName());
            return authcInfo;
        }
...
//            System.out.println("修改Session超时时间为[" + session.getTimeout() + "]毫秒");
     ...
     * 获取用户名和密码
     */
    private static UserModel getUserInfo(String mail){
    	UserModel um = new UserModel();
    	Connection conn = MysqlConnection.getConnection();
		// 通过数据的连接操作数据库
		String sql = "SELECT password,realname FROM userinformation where email='"+mail+"'";
		Statement stmt;
		...
		try {
			...
			while (result.next()) {
				...
				um.setPassword(result.getString("password"));
			}
