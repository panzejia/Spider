...
public class Parser {
	...
	public static class TitleParser {
		public static void main(String[] args) {
			String text = "广东省哲学社会科学“十三五”规划2017年度项目申报通知";
		...
		public static boolean projectJudgment(String text) {
			...
			try {
				// 创建分词对象
				Analyzer anal = new HanLPIndexAnalyzer();
				if (text == null) {
					anal.close();
					return false;
				}
...
//					"召开", "评估" };// 常用项目判定词
	...
	public static class UrlParser {
		...
		public static Stack<String> getArticleNoExistedList(List<String> oldUrlList, String source) {
			...
			urlMap = ArticleSqlConfiguration.getAllUrlByCagetory(source);
			Stack<String> urlList = new Stack<String>();
			if(oldUrlList==null){
				return (Stack<String>) oldUrlList;
			}
	 ...
	 * 地区解析器
	 */
	public static class AreaPaser{
		
		public static String getArea(String content){
			String area = "";
			...
			for (Term e : termList) {
				if (e.nature.toString().equals("ns")) {
					
					name.add(e.word);
				}
			}
			if(name.size()==0){
				if(checkNullArea(content)){
					return "";
				}else{
					return "系统暂未检测";
				}
		 ...
		 * @return
		 */
		private static   String getArea(ArrayList<String> name) {
			ArrayList<AreaModel> areaModelList = new ArrayList<AreaModel>();
			for (String n : name) {
				AreaModel areaModel = AreaSqlConfiguration.getAreaModel(n);
				areaModelList.add(areaModel);
			}
			String parentId = "", area = "";
			for (AreaModel areaModel : areaModelList) {
				if(areaModel.getParentId()==null){
					continue;
				}
				if (areaModel.getParentId().equals("1")) {	//解决同时出现“广东珠海”的情况
					parentId = areaModel.getId();
...
//					area = areaModel.getName();
				...
				if(parentId.equals("")){	//如果没有父地区就将该地区设为地区
					area = areaModel.getName();
				}
				if (areaModel.getParentId().equals(parentId)) {//解决同时出现“广东珠海”的情况
					area = areaModel.getName();
				}
				if (areaModel.getParentId().equals("1")) {//解决省级的问题
					area = areaModel.getName();
				}
		...
		private static boolean checkNullArea(String content){
			String[] stopwords = {"省","市"};
