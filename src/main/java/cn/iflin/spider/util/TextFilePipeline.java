...
public class TextFilePipeline extends FilePersistentBase implements Pipeline {
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        try {
            PrintWriter printWriter = new PrintWriter(new OutputStreamWriter(new FileOutputStream(getFile(path + SpiderParser.delSpecialSymbol(resultItems.getRequest().getUrl()) + ".txt")),"UTF-8"));
